...
```console
docker run -it --name fluxcd-source-controller bitmoa/fluxcd-source-controller
```

### Docker Compose

```console
curl -sSL https://raw.githubusercontent.com/bitmoa/containers/main/bitmoa/fluxcd-source-controller/docker-compose.yml > docker-compose.yml
docker-compose up -d
```
...
The recommended way to get the Bitnami Flux Docker Image is to pull the prebuilt image from the [Docker Hub Registry](https://hub.docker.com/r/bitmoa/fluxcd-source-controller).

```console
docker pull bitmoa/fluxcd-source-controller:latest
```

To use a specific version, you can pull a versioned tag. You can view the [list of available versions](https://hub.docker.com/r/bitmoa/fluxcd-source-controller/tags/) in the Docker Hub Registry.

```console
docker pull bitmoa/fluxcd-source-controller:[TAG]
```
...
or if you're using Docker Compose, update the value of the image property to `bitmoa/fluxcd-source-controller:latest`.
...
```console
docker run --name fluxcd-source-controller bitmoa/fluxcd-source-controller:latest
```
...
```console
docker run --rm --name fluxcd-source-controller bitmoa/fluxcd-source-controller:latest --help
```
...
The Bitnami Flux Docker image from the [Bitnami Secure Images](https://www.arrow.com/globalecs/uk/products/bitmoa-secure-images/) catalog includes extra features and settings to configure the container with FIPS capabilities. You can configure the next environment variables:
