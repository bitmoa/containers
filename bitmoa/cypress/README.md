...
```console
docker run -it --name cypress bitmoa/cypress
```
...
The recommended way to get the Bitnami cypress Docker Image is to pull the prebuilt image from the [Docker Hub Registry](https://hub.docker.com/r/bitmoa/cypress).

```console
docker pull bitmoa/cypress:latest
```

To use a specific version, you can pull a versioned tag. You can view the [list of available versions](https://hub.docker.com/r/bitmoa/cypress/tags/) in the Docker Hub Registry.

```console
docker pull bitmoa/cypress:[TAG]
```
...
```console
docker run -it --name cypress -v /path/to/app:/app bitmoa/cypress
```
...
The Bitnami Cypress Docker image from the [Bitnami Secure Images](https://www.arrow.com/globalecs/uk/products/bitmoa-secure-images/) catalog includes extra features and settings to configure the container with FIPS capabilities. You can configure the next environment variables:
...
```console
docker run --name cypress bitmoa/cypress:latest
```
