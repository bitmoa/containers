...
```console
docker run --name rabbitmq-cluster-operator bitmoa/rabbitmq-cluster-operator:latest
```
...
The recommended way to get the Bitnami rabbitmq-cluster-operator Docker Image is to pull the prebuilt image from the [Docker Hub Registry](https://hub.docker.com/r/bitmoa/rabbitmq-cluster-operator).

```console
docker pull bitmoa/rabbitmq-cluster-operator:latest
```

To use a specific version, you can pull a versioned tag. You can view the [list of available versions](https://hub.docker.com/r/bitmoa/rabbitmq-cluster-operator/tags/) in the Docker Hub Registry.

```console
docker pull bitmoa/rabbitmq-cluster-operator:[TAG]
```
...
```console
docker run --rm --name rabbitmq-cluster-operator bitmoa/rabbitmq-cluster-operator:latest --  --metrics-bind-address :9782
```
