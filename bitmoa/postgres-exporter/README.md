...
```console
docker run --name postgres-exporter bitmoa/postgres-exporter:latest
```
...
The recommended way to get the Bitnami PostgreSQL Exporter Docker Image is to pull the prebuilt image from the [Docker Hub Registry](https://hub.docker.com/r/bitmoa/postgres-exporter).

```console
docker pull bitmoa/postgres-exporter:latest
```

To use a specific version, you can pull a versioned tag. You can view the [list of available versions](https://hub.docker.com/r/bitmoa/postgres-exporter/tags/) in the Docker Hub Registry.

```console
docker pull bitmoa/postgres-exporter:[TAG]
```
...
```console
docker run --name postgres-exporter-node1 --network postgres-exporter-network bitmoa/postgres-exporter:latest
```
...
The Bitnami PostgreSQL Exporter Docker image from the [Bitnami Secure Images](https://www.arrow.com/globalecs/uk/products/bitmoa-secure-images/) catalog includes extra features and settings to configure the container with FIPS capabilities. You can configure the next environment variables:
