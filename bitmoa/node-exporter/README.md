...
```console
docker run --name node-exporter bitmoa/node-exporter:latest
```
...
The recommended way to get the Bitnami Node Exporter Docker Image is to pull the prebuilt image from the [Docker Hub Registry](https://hub.docker.com/r/bitmoa/node-exporter).

```console
docker pull bitmoa/node-exporter:latest
```

To use a specific version, you can pull a versioned tag. You can view the [list of available versions](https://hub.docker.com/r/bitmoa/node-exporter/tags/) in the Docker Hub Registry.

```console
docker pull bitmoa/node-exporter:[TAG]
```
...
```console
docker run --name node-exporter-node1 --network node-exporter-network bitmoa/node-exporter:latest
```
...
The Bitnami Node Exporter Docker image from the [Bitnami Secure Images](https://www.arrow.com/globalecs/uk/products/bitmoa-secure-images/) catalog includes extra features and settings to configure the container with FIPS capabilities. You can configure the next environment variables:
