...
```console
docker run --name elasticsearch-exporter bitmoa/elasticsearch-exporter:latest
```
...
The recommended way to get the Bitnami Elasticsearch Exporter Docker Image is to pull the prebuilt image from the [Docker Hub Registry](https://hub.docker.com/r/bitmoa/elasticsearch-exporter).

```console
docker pull bitmoa/elasticsearch-exporter:latest
```

To use a specific version, you can pull a versioned tag. You can view the [list of available versions](https://hub.docker.com/r/bitmoa/elasticsearch-exporter/tags/) in the Docker Hub Registry.

```console
docker pull bitmoa/elasticsearch-exporter:[TAG]
```
...
```console
docker run --name elasticsearch-exporter-node1 --network elasticsearch-exporter-network bitmoa/elasticsearch-exporter:latest
```
...
The Bitnami Elasticsearch Exporter Docker image from the [Bitnami Secure Images](https://www.arrow.com/globalecs/uk/products/bitmoa-secure-images/) catalog includes extra features and settings to configure the container with FIPS capabilities. You can configure the next environment variables:
