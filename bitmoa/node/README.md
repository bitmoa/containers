...
```console
docker run -it --name node bitmoa/node:latest
```
...
The recommended way to get the Bitnami Node.js Docker Image is to pull the prebuilt image from the [Docker Hub Registry](https://hub.docker.com/r/bitmoa/node).

```console
docker pull bitmoa/node:latest
```

To use a specific version, you can pull a versioned tag. You can view the [list of available versions](https://hub.docker.com/r/bitmoa/node/tags/) in the Docker Hub Registry.

```console
docker pull bitmoa/node:[TAG]
```
...
```console
docker run -it --name node bitmoa/node
```
...
```console
docker run -it --name node -v /path/to/app:/app bitmoa/node \
  node script.js
```
...
```console
docker run --rm -v /path/to/app:/app bitmoa/node npm install
docker run -it --name node  -v /path/to/app:/app bitmoa/node npm start
```

or by modifying the [`docker-compose.yml`](https://github.com/bitmoa/containers/blob/main/bitmoa/node/docker-compose.yml) file present in this repository:
...
The Bitnami Node.js Docker image from the [Bitnami Secure Images](https://www.arrow.com/globalecs/uk/products/bitmoa-secure-images/) catalog includes extra features and settings to configure the container with FIPS capabilities. You can configure the next environment variables:
...
```console
docker run -it --name node -v /path/to/app:/app -P bitmoa/node node index.js
```
...
```console
docker run -it --name node -p 8080:3000 -v /path/to/app:/app bitmoa/node node index.js
```
...
```console
docker run -it \
  -v /path/to/vhost.conf:/bitmoa/nginx/conf/vhosts/yourapp.conf:ro \
  --network app-tier \
  bitmoa/nginx
```
...
```console
docker run --name node bitmoa/node:latest
```
...
- Permissions fixed so `bitmoa` user can install global npm modules without needing `sudo`.
