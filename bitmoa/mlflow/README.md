...
```console
docker run -it --name mlflow bitmoa/mlflow:latest
```
...
The recommended way to get the Bitnami Mlflow Docker Image is to pull the prebuilt image from the [Docker Hub Registry](https://hub.docker.com/r/bitmoa/mlflow).

```console
docker pull bitmoa/mlflow:latest
```

To use a specific version, you can pull a versioned tag. You can view the [list of available versions](https://hub.docker.com/r/bitmoa/mlflow/tags/) in the Docker Hub Registry.

```console
docker pull bitmoa/mlflow:[TAG]
```
...
```console
docker run -it --name mlflow bitmoa/mlflow
```
...
```console
docker run -it --name mlflow -v /path/to/app:/app bitmoa/mlflow \
  python script.py
```
...
The Bitnami MLflow Docker image from the [Bitnami Secure Images](https://www.arrow.com/globalecs/uk/products/bitmoa-secure-images/) catalog includes extra features and settings to configure the container with FIPS capabilities. You can configure the next environment variables:
...
```console
docker run --name mlflow bitmoa/mlflow:latest
```
