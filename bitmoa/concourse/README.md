...
```console
docker run --name concourse bitmoa/concourse:latest
```

**Warning**: This quick setup is only intended for development environments. You are encouraged to change the insecure default credentials and check out the available configuration options for the [PostgreSQL container](https://github.com/bitmoa/containers/tree/main/bitmoa/postgresql#readme) for a more secure deployment.
...
The recommended way to get the Bitnami concourse Docker Image is to pull the prebuilt image from the [Docker Hub Registry](https://hub.docker.com/r/bitmoa/concourse).

```console
docker pull bitmoa/concourse:latest
```

To use a specific version, you can pull a versioned tag. You can view the [list of available versions](https://hub.docker.com/r/bitmoa/concourse/tags/) in the Docker Hub Registry.

```console
docker pull bitmoa/concourse:[TAG]
```
...
```console
docker run \
    -v /path/to/concourse-persistence:/bitmoa/concourse \
    bitmoa/concourse:latest
```

You can also do this with a minor change to the [`docker-compose.yml`](https://github.com/bitmoa/containers/blob/main/bitmoa/concourse/docker-compose.yml) file present in this repository:

```yaml
concourse:
  ...
  volumes:
    - /path/to/concourse-persistence:/bitmoa/concourse
  ...
```
...
```console
docker run --name concourse-node1 --network concourse-network bitmoa/concourse:latest
```
...
The Bitnami Concourse Docker image from the [Bitnami Secure Images](https://www.arrow.com/globalecs/uk/products/bitmoa-secure-images/) catalog includes extra features and settings to configure the container with FIPS capabilities. You can configure the next environment variables:
...
Please be aware this file has not undergone internal testing. Consequently, we advise its use exclusively for development or testing purposes. For production-ready deployments, we highly recommend utilizing its associated [Bitnami Helm chart](https://github.com/bitmoa/charts/tree/main/bitmoa/concourse).
