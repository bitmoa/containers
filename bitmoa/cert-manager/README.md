...
```console
docker run --name cert-manager -e ALLOW_EMPTY_PASSWORD=yes bitmoa/cert-manager:latest
```
...
The Bitnami cert-manager Docker image from the [Bitnami Secure Images](https://www.arrow.com/globalecs/uk/products/bitmoa-secure-images/) catalog includes extra features and settings to configure the container with FIPS capabilities. You can configure the next environment variables:
