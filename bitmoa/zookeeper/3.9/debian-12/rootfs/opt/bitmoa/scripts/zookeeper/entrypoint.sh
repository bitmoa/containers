...
# case someone mounts a configuration file in /opt/bitmoa/zookeeper/conf)
debug "Copying files from $ZOO_DEFAULT_CONF_DIR to $ZOO_CONF_DIR"
cp -nr "$ZOO_DEFAULT_CONF_DIR"/. "$ZOO_CONF_DIR"

if [[ "$*" = *"/opt/bitmoa/scripts/zookeeper/run.sh"* || "$*" = *"/run.sh"* ]]; then
    info "** Starting ZooKeeper setup **"
    /opt/bitmoa/scripts/zookeeper/setup.sh
