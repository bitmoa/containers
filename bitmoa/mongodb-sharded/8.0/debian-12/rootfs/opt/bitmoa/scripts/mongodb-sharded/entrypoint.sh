...
# case someone mounts a configuration file in /opt/bitmoa/mongodb/conf)
debug "Copying files from $MONGODB_DEFAULT_CONF_DIR to $MONGODB_CONF_DIR"
cp -nr "$MONGODB_DEFAULT_CONF_DIR"/. "$MONGODB_CONF_DIR"

if [[ "$*" = *"/opt/bitmoa/scripts/mongodb-sharded/run.sh"* || "$*" = *"/run.sh"* ]]; then
    info "** Starting MongoDB Sharded setup **"
    /opt/bitmoa/scripts/mongodb-sharded/setup.sh
