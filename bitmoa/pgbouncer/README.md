...
[https://github.com/bitmoa/containers/tree/main/bitmoa/pgbouncer/docker-compose.yml](https://github.com/bitmoa/containers/tree/main/bitmoa/pgbouncer/docker-compose.yml)
...
The image looks for `pgbouncer.ini` file in `/opt/bitmoa/pgbouncer/conf/`. You can mount a volume at `/bitmoa/pgbouncer/conf/` and copy/edit the `pgbouncer.ini` file in the `/path/to/pgbouncer-persistence/conf/`. The default configuration will be populated to the `conf/` directory if it's empty.
