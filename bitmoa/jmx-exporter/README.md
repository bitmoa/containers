...
```console
docker run --name jmx-exporter bitmoa/jmx-exporter:latest
```
...
The recommended way to get the Bitnami JMX Exporter Docker Image is to pull the prebuilt image from the [Docker Hub Registry](https://hub.docker.com/r/bitmoa/jmx-exporter).

```console
docker pull bitmoa/jmx-exporter:latest
```

To use a specific version, you can pull a versioned tag. You can view the [list of available versions](https://hub.docker.com/r/bitmoa/jmx-exporter/tags/) in the Docker Hub Registry.

```console
docker pull bitmoa/jmx-exporter:[TAG]
```
...
```console
docker run --name jmx-exporter-node1 --network jmx-exporter-network bitmoa/jmx-exporter:latest
```
...
The Bitnami JMX Exporter Docker image from the [Bitnami Secure Images](https://www.arrow.com/globalecs/uk/products/bitmoa-secure-images/) catalog includes extra features and settings to configure the container with FIPS capabilities. You can configure the next environment variables:
