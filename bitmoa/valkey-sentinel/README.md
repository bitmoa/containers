...
```console
docker run --name valkey-sentinel -e VALKEY_PRIMARY_HOST=valkey bitmoa/valkey-sentinel:latest
```
...
The recommended way to get the Bitnami Valkey Sentinel Docker Image is to pull the prebuilt image from the [Docker Hub Registry](https://hub.docker.com/r/bitmoa/valkey-sentinel).

```console
docker pull bitmoa/valkey-sentinel:latest
```

To use a specific version, you can pull a versioned tag. You can view the [list of available versions](https://hub.docker.com/r/bitmoa/valkey-sentinel/tags/) in the Docker Hub Registry.

```console
docker pull bitmoa/valkey-sentinel:[TAG]
```
...
|---------------------------------------|----------------------------------------|--------------------------------------------------|
| `VALKEY_SENTINEL_VOLUME_DIR`          | Persistence base directory             | `/bitmoa/valkey-sentinel`                       |
| `VALKEY_SENTINEL_BASE_DIR`            | Valkey installation directory          | `${BITMOA_ROOT_DIR}/valkey-sentinel`            |
...
1. Using `docker run`
    ...
    $ docker run --name valkey-sentinel \
        ...
        -v /path/to/valkey-sentinel/persistence:/bitmoa \
        -e VALKEY_PRIMARY_HOST=valkey \
        -e VALKEY_SENTINEL_TLS_ENABLED=yes \
        -e VALKEY_SENTINEL_TLS_CERT_FILE=/opt/bitmoa/valkey/certs/valkey.crt \
        -e VALKEY_SENTINEL_TLS_KEY_FILE=/opt/bitmoa/valkey/certs/valkey.key \
        -e VALKEY_SENTINEL_TLS_CA_FILE=/opt/bitmoa/valkey/certs/valkeyCA.crt \
        bitmoa/valkey-cluster:latest
        bitmoa/valkey-sentinel:latest
    ```

Alternatively, you may also provide with this configuration in your [custom](https://github.com/bitmoa/containers/blob/main/bitmoa/valkey-sentinel#configuration-file) configuration file.

### Configuration file

The image looks for configurations in `/bitmoa/valkey-sentinel/conf/`. You can mount a volume at `/bitmoa` and copy/edit the configurations in the `/path/to/valkey-persistence/valkey-sentinel/conf/`. The default configurations will be populated to the `conf/` directory if it's empty.
...
The Bitnami Valkey Sentinel Docker image from the [Bitnami Secure Images](https://www.arrow.com/globalecs/uk/products/bitmoa-secure-images/) catalog includes extra features and settings to configure the container with FIPS capabilities. You can configure the next environment variables:
...
```console
docker run --name valkey bitmoa/valkey-sentinel:latest
```
