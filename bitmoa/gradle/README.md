...
```console
docker run -it --name gradle bitmoa/gradle:latest
```
...
The recommended way to get the Bitnami Gradle Docker Image is to pull the prebuilt image from the [Docker Hub Registry](https://hub.docker.com/r/bitmoa/gradle).

```console
docker pull bitmoa/gradle:latest
```

To use a specific version, you can pull a versioned tag. You can view the [list of available versions](https://hub.docker.com/r/bitmoa/gradle/tags/) in the Docker Hub Registry.

```console
docker pull bitmoa/gradle:[TAG]
```
...
```console
docker run --name gradle -v /path/to/app:/app bitmoa/gradle \
  build
```
...
The Bitnami Gradle Docker image from the [Bitnami Secure Images](https://www.arrow.com/globalecs/uk/products/bitmoa-secure-images/) catalog includes extra features and settings to configure the container with FIPS capabilities. You can configure the next environment variables:
...
```console
docker run --name gradle bitmoa/gradle:latest
```
