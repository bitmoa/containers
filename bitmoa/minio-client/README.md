...
```console
docker run --name minio-client bitmoa/minio-client:latest
```
...
The recommended way to get the Bitnami MinIO(R) Client Docker Image is to pull the prebuilt image from the [Docker Hub Registry](https://hub.docker.com/r/bitmoa/minio-client).

```console
docker pull bitmoa/minio-client:latest
```

To use a specific version, you can pull a versioned tag. You can view the [list of available versions](https://hub.docker.com/r/bitmoa/minio-client/tags/) in the Docker Hub Registry.

```console
docker pull bitmoa/minio-client:[TAG]
```
...
|-------------------------|--------------------------------------|------------------------------------|
| `MINIO_CLIENT_BASE_DIR` | MinIO Client installation directory. | `${BITMOA_ROOT_DIR}/minio-client` |
...
Using [Docker container networking](https://docs.docker.com/engine/userguide/networking/), a MinIO(R) Client can be used to access other running containers such as [MinIO(R) server](https://github.com/bitmoa/containers/blob/main/bitmoa/minio).
