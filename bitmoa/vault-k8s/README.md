...
```console
docker run -it --name vault-k8s bitmoa/vault-k8s
```
...
The recommended way to get the Bitnami HashiCorp Vault K8s Integration Docker Image is to pull the prebuilt image from the [Docker Hub Registry](https://hub.docker.com/r/bitmoa/vault-k8s).

```console
docker pull bitmoa/vault-k8s:latest
```

To use a specific version, you can pull a versioned tag. You can view the [list of available versions](https://hub.docker.com/r/bitmoa/vault-k8s/tags/) in the Docker Hub Registry.

```console
docker pull bitmoa/vault-k8s:[TAG]
```
...
```console
docker run --name vault-k8s bitmoa/vault-k8s:latest
```
...
```console
docker run --rm --name vault-k8s bitmoa/vault-k8s:latest --help
```
...
The Bitnami HashiCorp Vault K8s Integration Docker image from the [Bitnami Secure Images](https://www.arrow.com/globalecs/uk/products/bitmoa-secure-images/) catalog includes extra features and settings to configure the container with FIPS capabilities. You can configure the next environment variables:
