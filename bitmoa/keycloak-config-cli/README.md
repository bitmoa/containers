...
```console
docker run --rm --name keycloak-config-cli bitmoa/keycloak-config-cli:latest
```
...
The recommended way to get the Bitnami Keycloak Config CLI Docker Image is to pull the prebuilt image from the [Docker Hub Registry](https://hub.docker.com/r/bitmoa/keycloak-config-cli).

```console
docker pull bitmoa/keycloak-config-cli:latest
```

To use a specific version, you can pull a versioned tag. You can view the [list of available versions](https://hub.docker.com/r/bitmoa/keycloak-config-cli/tags/) in the Docker Hub Registry.

```console
docker pull bitmoa/keycloak-config-cli:[TAG]
```
...
The Bitnami Keycloak Config CLI Docker image from the [Bitnami Secure Images](https://www.arrow.com/globalecs/uk/products/bitmoa-secure-images/) catalog includes extra features and settings to configure the container with FIPS capabilities. You can configure the next environment variables:
