...
```console
docker run -it --name jax bitmoa/jax
```
...
The recommended way to get the Bitnami Jax Docker Image is to pull the prebuilt image from the [Docker Hub Registry](https://hub.docker.com/r/bitmoa/jax).

```console
docker pull bitmoa/jax:latest
```

To use a specific version, you can pull a versioned tag. You can view the [list of available versions](https://hub.docker.com/r/bitmoa/jax/tags/) in the Docker Hub Registry.

```console
docker pull bitmoa/jax:[TAG]
```
...
```console
docker run -it --name jax -v /path/to/app:/app bitmoa/jax \
  python script.py
```
...
The Bitnami JAX Docker image from the [Bitnami Secure Images](https://www.arrow.com/globalecs/uk/products/bitmoa-secure-images/) catalog includes extra features and settings to configure the container with FIPS capabilities. You can configure the next environment variables:
...
```console
docker run --name jax bitmoa/jax:latest
```
