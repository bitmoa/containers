...
if [[ "$1" = "/opt/bitmoa/scripts/suitecrm/run.sh" || "$1" = "/opt/bitmoa/scripts/$(web_server_type)/run.sh" || "$1" = "/opt/bitmoa/scripts/nginx-php-fpm/run.sh" ]]; then
    ...
    /opt/bitmoa/scripts/suitecrm/setup.sh
    /post-init.sh
