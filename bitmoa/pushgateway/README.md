...
```console
docker run --name pushgateway bitmoa/pushgateway:latest
```
...
The recommended way to get the Bitnami Pushgateway Docker Image is to pull the prebuilt image from the [Docker Hub Registry](https://hub.docker.com/r/bitmoa/pushgateway).

```console
docker pull bitmoa/pushgateway:latest
```

To use a specific version, you can pull a versioned tag. You can view the [list of available versions](https://hub.docker.com/r/bitmoa/pushgateway/tags/) in the Docker Hub Registry.

```console
docker pull bitmoa/pushgateway:[TAG]
```
...
```console
docker run --name pushgateway-node1 --network pushgateway-network bitmoa/pushgateway:latest
```
...
The Bitnami Push Gateway Docker image from the [Bitnami Secure Images](https://www.arrow.com/globalecs/uk/products/bitmoa-secure-images/) catalog includes extra features and settings to configure the container with FIPS capabilities. You can configure the next environment variables:
