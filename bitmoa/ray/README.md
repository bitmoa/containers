...
```console
docker run -it --name ray bitmoa/ray
```
...
The recommended way to get the Bitnami Ray Docker Image is to pull the prebuilt image from the [Docker Hub Registry](https://hub.docker.com/r/bitmoa/ray).

```console
docker pull bitmoa/ray:latest
```

To use a specific version, you can pull a versioned tag. You can view the [list of available versions](https://hub.docker.com/r/bitmoa/ray/tags/) in the Docker Hub Registry.

```console
docker pull bitmoa/ray:[TAG]
```
...
```console
docker run -it --name ray -v /path/to/app:/app bitmoa/ray \
  python script.py
```
...
The Bitnami Ray Docker image from the [Bitnami Secure Images](https://www.arrow.com/globalecs/uk/products/bitmoa-secure-images/) catalog includes extra features and settings to configure the container with FIPS capabilities. You can configure the next environment variables:
...
```console
docker run --name ray bitmoa/ray:latest
```
