...
```console
docker run -it --name cosign bitmoa/cosign
```
...
The recommended way to get the Bitnami Cosign Docker Image is to pull the prebuilt image from the [Docker Hub Registry](https://hub.docker.com/r/bitmoa/cosign).

```console
docker pull bitmoa/cosign:latest
```

To use a specific version, you can pull a versioned tag. You can view the [list of available versions](https://hub.docker.com/r/bitmoa/cosign/tags/) in the Docker Hub Registry.

```console
docker pull bitmoa/cosign:[TAG]
```
...
```console
docker run --name cosign bitmoa/cosign:latest
```
...
```console
docker run --rm --name cosign bitmoa/cosign:latest --help
```
...
The Bitnami Cosign Docker image from the [Bitnami Secure Images](https://www.arrow.com/globalecs/uk/products/bitmoa-secure-images/) catalog includes extra features and settings to configure the container with FIPS capabilities. You can configure the next environment variables:
