...
```console
docker run -it --name superset bitmoa/superset
```
...
The recommended way to get the Bitnami Superset Docker Image is to pull the prebuilt image from the [Docker Hub Registry](https://hub.docker.com/r/bitmoa/superset).

```console
docker pull bitmoa/superset:latest
```

To use a specific version, you can pull a versioned tag. You can view the [list of available versions](https://hub.docker.com/r/bitmoa/superset/tags/) in the Docker Hub Registry.

```console
docker pull bitmoa/superset:[TAG]
```
...
```console
docker run -it --name superset -v /path/to/app:/app bitmoa/superset \
  python script.py
```

### Running a Apache Superset app with package dependencies

If your Apache Superset app has a `requirements.txt` defining your app's dependencies, you can install the dependencies before running your app.
...
The Bitnami Apache Superset Docker image from the [Bitnami Secure Images](https://www.arrow.com/globalecs/uk/products/bitmoa-secure-images/) catalog includes extra features and settings to configure the container with FIPS capabilities. You can configure the next environment variables:
...
```console
docker run --name superset bitmoa/superset:latest
```
