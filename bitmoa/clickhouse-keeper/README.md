...
```console
docker run --name clickhouse-keeper bitmoa/clickhouse-keeper:latest
```
...
The recommended way to get the Bitnami ClickHouse Keeper Docker Image is to pull the prebuilt image from the [Docker Hub Registry](https://hub.docker.com/r/bitmoa/clickhouse-keeper).

```console
docker pull bitmoa/clickhouse-keeper:latest
```

To use a specific version, you can pull a versioned tag. You can view the [list of available versions](https://hub.docker.com/r/bitmoa/clickhouse-keeper/tags/) in the Docker Hub Registry.

```console
docker pull bitmoa/clickhouse-keeper:[TAG]
```
...
For persistence you should mount a directory at the `/bitmoa/clickhouse-keeper` path. If the mounted directory is empty, it will be initialized on the first run.

```console
docker run \
    --volume /path/to/clickhouse-keeper-persistence:/bitmoa/clickhouse-keeper \
    bitmoa/clickhouse-keeper:latest
```

You can also do this with a minor change to the [`docker-compose.yml`](https://github.com/bitmoa/containers/blob/main/bitmoa/clickhouse-keeper/docker-compose.yml) file present in this repository:

```console
clickhouse-keeper:
  ...
  volumes:
    - /path/to/clickhouse-keeper-persistence:/bitmoa/clickhouse-keeper
  ...
```
...
docker run -it --rm \
    --network my-network \
    bitmoa/clickhouse-keeper:latest clickhouse-keeper-client --host clickhouse-keeper
```
...
services:
  clickhouse-keeper:
    image: bitmoa/clickhouse-keeper:latest
    environment:
...
The configuration can easily be setup by mounting your own configuration overrides on the directory `/bitmoa/clickhouse-keeper/etc/config.d` or `/bitmoa/clickhouse-keeper/etc/users.d`:

```console
docker run --name clickhouse-keeper \
    --volume /path/to/override.xml:/bitmoa/clickhouse-keeper/etc/config.d/override.xml:ro \
    bitmoa/clickhouse-keeper:latest
```
...
services:
  clickhouse-keeper:
    image: bitmoa/clickhouse-keeper:latest
    volumes:
      - /path/to/override.xml:/bitmoa/clickhouse-keeper/etc/config.d/override.xml:ro
```
...
|-----------------------------------------|-----------------------------------------------------|----------------------------------------------------------|
| `CLICKHOUSE_KEEPER_BASE_DIR`            | ClickHouse Keeper installation directory.           | `${BITMOA_ROOT_DIR}/clickhouse-keeper`                  |
| `CLICKHOUSE_KEEPER_VOLUME_DIR`          | ClickHouse Keeper volume directory.                 | `/bitmoa/clickhouse-keeper`                             |
...
The Bitnami ClickHouse Keeper Docker image from the [Bitnami Secure Images](https://www.arrow.com/globalecs/uk/products/bitmoa-secure-images/) catalog includes extra features and settings to configure the container with FIPS capabilities. You can configure the next environment variables:
...
or if you're using Docker Compose, update the value of the image property to `bitmoa/clickhouse-keeper:latest`.
