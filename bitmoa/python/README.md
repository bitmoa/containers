...
```console
docker run -it --name python bitmoa/python
```
...
The recommended way to get the Bitnami Python Docker Image is to pull the prebuilt image from the [Docker Hub Registry](https://hub.docker.com/r/bitmoa/python).

```console
docker pull bitmoa/python:latest
```

To use a specific version, you can pull a versioned tag. You can view the [list of available versions](https://hub.docker.com/r/bitmoa/python/tags/) in the Docker Hub Registry.

```console
docker pull bitmoa/python:[TAG]
```
...
```console
docker run -it --name python -v /path/to/app:/app bitmoa/python \
  python script.py
```
...
```console
docker run --rm -v /path/to/app:/app bitmoa/python pip install -r requirements.txt
docker run -it --name python -v /path/to/app:/app bitmoa/python python script.py
```
...
The Bitnami Python Docker image from the [Bitnami Secure Images](https://www.arrow.com/globalecs/uk/products/bitmoa-secure-images/) catalog includes extra features and settings to configure the container with FIPS capabilities. You can configure the next environment variables:
...
or if you're using Docker Compose, update the value of the image property to `bitmoa/python:latest`.
...
```console
docker run --name python bitmoa/python:latest
```
