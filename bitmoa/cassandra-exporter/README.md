...
```console
docker run --name cassandra-exporter bitmoa/cassandra-exporter:latest
```
...
The recommended way to get the Bitnami Cassandra Exporter Docker Image is to pull the prebuilt image from the [Docker Hub Registry](https://hub.docker.com/r/bitmoa/cassandra-exporter).

```console
docker pull bitmoa/cassandra-exporter:latest
```

To use a specific version, you can pull a versioned tag. You can view the [list of available versions](https://hub.docker.com/r/bitmoa/cassandra-exporter/tags/) in the Docker Hub Registry.

```console
docker pull bitmoa/cassandra-exporter:[TAG]
```
...
```console
docker run --name cassandra-exporter-node1 --network cassandra-exporter-network bitmoa/cassandra-exporter:latest
```
...
The Bitnami Cassandra Exporter Docker image from the [Bitnami Secure Images](https://www.arrow.com/globalecs/uk/products/bitmoa-secure-images/) catalog includes extra features and settings to configure the container with FIPS capabilities. You can configure the next environment variables:
