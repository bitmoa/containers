...
```console
docker run --name tensorflow-serving bitmoa/tensorflow-serving:latest
```
...
The recommended way to get the Bitnami TensorFlow Serving Docker Image is to pull the prebuilt image from the [Docker Hub Registry](https://hub.docker.com/r/bitmoa/tensorflow-serving).

```console
docker pull bitmoa/tensorflow-serving:latest
```

To use a specific version, you can pull a versioned tag. You can view the [list of available versions](https://hub.docker.com/r/bitmoa/tensorflow-serving/tags/) in the Docker Hub Registry.

```console
docker pull bitmoa/tensorflow-serving:[TAG]
```
...
For persistence you should mount a volume at the `/bitmoa` path for the TensorFlow Serving data and configurations. If the mounted directory is empty, it will be initialized on the first run.

```console
docker run -v /path/to/tensorflow-serving-persistence:/bitmoa bitmoa/tensorflow-serving:latest
```

Alternatively, modify the [`docker-compose.yml`](https://github.com/bitmoa/containers/blob/main/bitmoa/tensorflow-serving/docker-compose.yml) file present in this repository:
...
services:
  ...
  ...
    volumes:
      - /path/to/tensorflow-serving-persistence:/bitmoa
  ...
```
...
docker run -d --name tensorflow-serving \
    --volume /tmp/model-data:/bitmoa/model-data \
    --network app-tier \
    bitmoa/tensorflow-serving:latest
```
...
docker run -d --name tensorflow-resnet \
    --volume /tmp/model-data:/bitmoa/model-data \
    --network app-tier \
    bitmoa/tensorflow-resnet:latest
```
...
docker run -it --rm \
    --volume /tmp/model-data:/bitmoa/model-data \
    --network app-tier \
    bitmoa/tensorflow-resnet:latest resnet_client_cc --server_port=tensorflow-serving:8500 --image_file=path/to/image.jpg
```
...
services:
  tensorflow-serving:
    image: bitmoa/tensorflow-serving:latest
    networks:
      - app-tier
  myapp:
...
| `TENSORFLOW_SERVING_BASE_DIR`             | Tensorflow installation directory.            | `${BITMOA_ROOT_DIR}/tensorflow-serving`                 |
...
| `TENSORFLOW_SERVING_VOLUME_DIR`           | Tensorflow persistence directory.             | `${BITMOA_VOLUME_DIR}/tensorflow-serving`               |
| `TENSORFLOW_SERVING_MODEL_DATA`           | Tensorflow data to persist.                   | `${BITMOA_VOLUME_DIR}/model-data`                       |
...
The image looks for configurations in `/bitmoa/tensorflow-serving/conf/`. As mentioned in [Persisting your configuation](#persisting-your-configuration) you can mount a volume at `/bitmoa` and copy/edit the configurations in the `/path/to/tensorflow-serving-persistence/tensorflow-serving/conf/`. The default configurations will be populated to the `conf/` directory if it's empty.
...
```console
docker run --name tensorflow-serving -v /path/to/tensorflow-serving-persistence:/bitmoa bitmoa/tensorflow-serving:latest
```
...
The Bitnami TensorFlow Serving Docker image from the [Bitnami Secure Images](https://www.arrow.com/globalecs/uk/products/bitmoa-secure-images/) catalog includes extra features and settings to configure the container with FIPS capabilities. You can configure the next environment variables:
...
The logs are also stored inside the container in the /opt/bitmoa/tensorflow-serving/logs/tensorflow-serving.log file.
