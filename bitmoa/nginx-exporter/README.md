...
```console
docker run --name nginx-exporter bitmoa/nginx-exporter:latest
```
...
The recommended way to get the Bitnami NGINX Exporter Docker Image is to pull the prebuilt image from the [Docker Hub Registry](https://hub.docker.com/r/bitmoa/nginx-exporter).

```console
docker pull bitmoa/nginx-exporter:latest
```

To use a specific version, you can pull a versioned tag. You can view the [list of available versions](https://hub.docker.com/r/bitmoa/nginx-exporter/tags/) in the Docker Hub Registry.

```console
docker pull bitmoa/nginx-exporter:[TAG]
```
...
```console
docker run --name nginx-exporter-node1 --network nginx-exporter-network bitmoa/nginx-exporter:latest
```
...
The Bitnami NGINX Exporter Docker image from the [Bitnami Secure Images](https://www.arrow.com/globalecs/uk/products/bitmoa-secure-images/) catalog includes extra features and settings to configure the container with FIPS capabilities. You can configure the next environment variables:
