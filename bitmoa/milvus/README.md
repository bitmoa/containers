...
```console
docker run -it --name milvus bitmoa/milvus
```
...
The recommended way to get the Bitnami Milvus Docker Image is to pull the prebuilt image from the [Docker Hub Registry](https://hub.docker.com/r/bitmoa/milvus).

```console
docker pull bitmoa/milvus:latest
```

To use a specific version, you can pull a versioned tag. You can view the [list of available versions](https://hub.docker.com/r/bitmoa/milvus/tags/) in the Docker Hub Registry.

```console
docker pull bitmoa/milvus:[TAG]
```
...
```console
docker run --name milvus bitmoa/milvus:latest
```
...
```console
docker run --rm --name milvus bitmoa/milvus:latest --help
```
...
The Bitnami Milvus Docker image from the [Bitnami Secure Images](https://www.arrow.com/globalecs/uk/products/bitmoa-secure-images/) catalog includes extra features and settings to configure the container with FIPS capabilities. You can configure the next environment variables:
