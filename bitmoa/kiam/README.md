...
```console
docker run --name kiam bitmoa/kiam:latest
```
...
The recommended way to get the Bitnami Kiam Docker Image is to pull the prebuilt image from the [Docker Hub Registry](https://hub.docker.com/r/bitmoa/kiam).

```console
docker pull bitmoa/kiam:latest
```

To use a specific version, you can pull a versioned tag. You can view the [list of available versions](https://hub.docker.com/r/bitmoa/kiam/tags/) in the Docker Hub Registry.

```console
docker pull bitmoa/kiam:[TAG]
```
...
```console
docker run --rm --name kiam bitmoa/kiam:latest -- --version
```
...
The Bitnami Kiam Docker image from the [Bitnami Secure Images](https://www.arrow.com/globalecs/uk/products/bitmoa-secure-images/) catalog includes extra features and settings to configure the container with FIPS capabilities. You can configure the next environment variables:
