...
# case someone mounts a configuration file at /opt/bitmoa/clickhouse-keeper/etc)
...
if [[ "$1" = "/opt/bitmoa/scripts/clickhouse-keeper/run.sh" ]]; then
    info "** Starting ClickHouse Keeper setup **"
    /opt/bitmoa/scripts/clickhouse-keeper/setup.sh
