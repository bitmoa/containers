...
```console
docker run -it --name zipkin bitmoa/zipkin
```
...
The recommended way to get the Bitnami zipkin Docker Image is to pull the prebuilt image from the [Docker Hub Registry](https://hub.docker.com/r/bitmoa/zipkin).

```console
docker pull bitmoa/zipkin:latest
```

To use a specific version, you can pull a versioned tag. You can view the [list of available versions](https://hub.docker.com/r/bitmoa/zipkin/tags/) in the Docker Hub Registry.

```console
docker pull bitmoa/zipkin:[TAG]
```
...
```console
docker run --name git bitmoa/zipkin:latest --version
```
...
The Bitnami Zipkin Docker image from the [Bitnami Secure Images](https://www.arrow.com/globalecs/uk/products/bitmoa-secure-images/) catalog includes extra features and settings to configure the container with FIPS capabilities. You can configure the next environment variables:
...
```console
docker run --name zipkin bitmoa/zipkin:latest
```
