...
```console
docker run --name telegraf bitmoa/telegraf:latest
```
...
The recommended way to get the Bitnami telegraf Docker Image is to pull the prebuilt image from the [Docker Hub Registry](https://hub.docker.com/r/bitmoa/telegraf).

```console
docker pull bitmoa/telegraf:latest
```

To use a specific version, you can pull a versioned tag. You can view the [list of available versions](https://hub.docker.com/r/bitmoa/telegraf/tags/) in the Docker Hub Registry.

```console
docker pull bitmoa/telegraf:[TAG]
```
...
```console
docker run --rm --name telegraf bitmoa/telegraf:latest -- telegraf --version
```
...
The Bitnami Telegraf &trade; Docker image from the [Bitnami Secure Images](https://www.arrow.com/globalecs/uk/products/bitmoa-secure-images/) catalog includes extra features and settings to configure the container with FIPS capabilities. You can configure the next environment variables:
