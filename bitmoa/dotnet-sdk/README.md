...
```console
docker run --name dotnet-sdk bitmoa/dotnet-sdk:latest
```
...
The recommended way to get the Bitnami .NET SDK Docker Image is to pull the prebuilt image from the [Docker Hub Registry](https://hub.docker.com/r/bitmoa/dotnet-sdk).

```console
docker pull bitmoa/dotnet-sdk:latest
```

To use a specific version, you can pull a versioned tag. You can view the [list of available versions](https://hub.docker.com/r/bitmoa/dotnet-sdk/tags/) in the Docker Hub Registry.

```console
docker pull bitmoa/dotnet-sdk:[TAG]
```
...
```console
docker run \
    -v /path/to/dotnet-persistence:/bitmoa \
    bitmoa/dotnet-sdk:latest
```

You can also do this with a minor change to the [`docker-compose.yml`](https://github.com/bitmoa/containers/blob/main/bitmoa/dotnet-sdk/docker-compose.yml) file present in this repository:
...
```console
docker run --name dotnet-node1 --network dotnet-network bitmoa/dotnet-sdk:latest
```
...
The Bitnami .NET SDK Docker image from the [Bitnami Secure Images](https://www.arrow.com/globalecs/uk/products/bitmoa-secure-images/) catalog includes extra features and settings to configure the container with FIPS capabilities. You can configure the next environment variables:
