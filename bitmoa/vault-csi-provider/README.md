...
```console
docker run -it --name vault-csi-provider bitmoa/vault-csi-provider
```
...
The recommended way to get the Bitnami HashiCorp Vault CSI Provider Docker Image is to pull the prebuilt image from the [Docker Hub Registry](https://hub.docker.com/r/bitmoa/vault-csi-provider).

```console
docker pull bitmoa/vault-csi-provider:latest
```

To use a specific version, you can pull a versioned tag. You can view the [list of available versions](https://hub.docker.com/r/bitmoa/vault-csi-provider/tags/) in the Docker Hub Registry.

```console
docker pull bitmoa/vault-csi-provider:[TAG]
```
...
```console
docker run --name vault-csi-provider bitmoa/vault-csi-provider:latest
```
...
```console
docker run --rm --name vault-csi-provider bitmoa/vault-csi-provider:latest --help
```
...
The Bitnami HashiCorp Vault CSI Provider Docker image from the [Bitnami Secure Images](https://www.arrow.com/globalecs/uk/products/bitmoa-secure-images/) catalog includes extra features and settings to configure the container with FIPS capabilities. You can configure the next environment variables:
