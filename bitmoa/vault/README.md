...
```console
docker run -it --name vault bitmoa/vault
```
...
The recommended way to get the Bitnami HashiCorp Vault Docker Image is to pull the prebuilt image from the [Docker Hub Registry](https://hub.docker.com/r/bitmoa/vault).

```console
docker pull bitmoa/vault:latest
```

To use a specific version, you can pull a versioned tag. You can view the [list of available versions](https://hub.docker.com/r/bitmoa/vault/tags/) in the Docker Hub Registry.

```console
docker pull bitmoa/vault:[TAG]
```
...
```console
docker run --name vault bitmoa/vault:latest
```
...
```console
docker run --rm --name vault bitmoa/vault:latest --help
```
...
The Bitnami HashiCorp Vault Docker image from the [Bitnami Secure Images](https://www.arrow.com/globalecs/uk/products/bitmoa-secure-images/) catalog includes extra features and settings to configure the container with FIPS capabilities. You can configure the next environment variables:
