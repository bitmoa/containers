...
```console
docker run -it --name pymilvus bitmoa/pymilvus
```
...
The recommended way to get the Bitnami Pymilvus Docker Image is to pull the prebuilt image from the [Docker Hub Registry](https://hub.docker.com/r/bitmoa/pymilvus).

```console
docker pull bitmoa/pymilvus:latest
```

To use a specific version, you can pull a versioned tag. You can view the [list of available versions](https://hub.docker.com/r/bitmoa/pymilvus/tags/) in the Docker Hub Registry.

```console
docker pull bitmoa/pymilvus:[TAG]
```
...
```console
docker run -it --name pymilvus -v /path/to/app:/app bitmoa/pymilvus \
  python script.py
```
...
The Bitnami PyMilvus Docker image from the [Bitnami Secure Images](https://www.arrow.com/globalecs/uk/products/bitmoa-secure-images/) catalog includes extra features and settings to configure the container with FIPS capabilities. You can configure the next environment variables:
...
```console
docker run --name pymilvus bitmoa/pymilvus:latest
```
