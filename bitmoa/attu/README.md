...
```console
docker run -it --name attu bitmoa/attu
```
...
The recommended way to get the Bitnami Attu Docker Image is to pull the prebuilt image from the [Docker Hub Registry](https://hub.docker.com/r/bitmoa/attu).

```console
docker pull bitmoa/attu:latest
```

To use a specific version, you can pull a versioned tag. You can view the [list of available versions](https://hub.docker.com/r/bitmoa/attu/tags/) in the Docker Hub Registry.

```console
docker pull bitmoa/attu:[TAG]
```
...
```console
docker run --name attu bitmoa/attu:latest
```
...
```console
docker run --rm --name attu bitmoa/attu:latest --help
```
...
The Bitnami Attu Docker image from the [Bitnami Secure Images](https://www.arrow.com/globalecs/uk/products/bitmoa-secure-images/) catalog includes extra features and settings to configure the container with FIPS capabilities. You can configure the next environment variables:
