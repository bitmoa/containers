...
```console
docker run -it --name pytorch bitmoa/pytorch
```
...
The recommended way to get the Bitnami Pytorch Docker Image is to pull the prebuilt image from the [Docker Hub Registry](https://hub.docker.com/r/bitmoa/pytorch).

```console
docker pull bitmoa/pytorch:latest
```

To use a specific version, you can pull a versioned tag. You can view the [list of available versions](https://hub.docker.com/r/bitmoa/pytorch/tags/) in the Docker Hub Registry.

```console
docker pull bitmoa/pytorch:[TAG]
```
...
```console
docker run -it --name pytorch -v /path/to/app:/app bitmoa/pytorch \
  python script.py
```
...
The Bitnami PyTorch Docker image from the [Bitnami Secure Images](https://www.arrow.com/globalecs/uk/products/bitmoa-secure-images/) catalog includes extra features and settings to configure the container with FIPS capabilities. You can configure the next environment variables:
...
or if you're using Docker Compose, update the value of the image property to `bitmoa/pytorch:latest`.
...
```console
docker run --name pytorch bitmoa/pytorch:latest
```
...
Please be aware this file has not undergone internal testing. Consequently, we advise its use exclusively for development or testing purposes. For production-ready deployments, we highly recommend utilizing its associated [Bitnami Helm chart](https://github.com/bitmoa/charts/tree/main/bitmoa/pytorch).
