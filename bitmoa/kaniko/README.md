...
```console
docker run -it --name kaniko bitmoa/kaniko
```
...
The recommended way to get the Bitnami Kaniko Docker Image is to pull the prebuilt image from the [Docker Hub Registry](https://hub.docker.com/r/bitmoa/kaniko).

```console
docker pull bitmoa/kaniko:latest
```

To use a specific version, you can pull a versioned tag. You can view the [list of available versions](https://hub.docker.com/r/bitmoa/kaniko/tags/) in the Docker Hub Registry.

```console
docker pull bitmoa/kaniko:[TAG]
```
...
```console
docker run --name kaniko bitmoa/kaniko:latest
```
...
```console
docker run --rm --name kaniko bitmoa/kaniko:latest --help
```
...
* The `bitmoa/kaniko` is now based on minimal Linux distribution instead of scratch. This change means the container now includes a shell and basic utilities, allowing you to easily debug and troubleshoot within the container.
