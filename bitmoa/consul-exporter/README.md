...
```console
docker run --name consul-exporter bitmoa/consul-exporter:latest
```
...
The recommended way to get the Bitnami Consul Exporter Docker Image is to pull the prebuilt image from the [Docker Hub Registry](https://hub.docker.com/r/bitmoa/consul-exporter).

```console
docker pull bitmoa/consul-exporter:latest
```

To use a specific version, you can pull a versioned tag. You can view the [list of available versions](https://hub.docker.com/r/bitmoa/consul-exporter/tags/) in the Docker Hub Registry.

```console
docker pull bitmoa/consul-exporter:[TAG]
```
...
```console
docker run --name consul-exporter-node1 --network consul-exporter-network bitmoa/consul-exporter:latest
```
...
The Bitnami Consul Exporter Docker image from the [Bitnami Secure Images](https://www.arrow.com/globalecs/uk/products/bitmoa-secure-images/) catalog includes extra features and settings to configure the container with FIPS capabilities. You can configure the next environment variables:
