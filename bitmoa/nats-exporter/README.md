...
```console
docker run --name nats-exporter bitmoa/nats-exporter:latest
```
...
The recommended way to get the Bitnami NATS Exporter Docker Image is to pull the prebuilt image from the [Docker Hub Registry](https://hub.docker.com/r/bitmoa/nats-exporter).

```console
docker pull bitmoa/nats-exporter:latest
```

To use a specific version, you can pull a versioned tag. You can view the [list of available versions](https://hub.docker.com/r/bitmoa/nats-exporter/tags/) in the Docker Hub Registry.

```console
docker pull bitmoa/nats-exporter:[TAG]
```
...
```console
docker run --name nats-exporter-node1 --network nats-exporter-network bitmoa/nats-exporter:latest
```
...
The Bitnami NATS Exporter Docker image from the [Bitnami Secure Images](https://www.arrow.com/globalecs/uk/products/bitmoa-secure-images/) catalog includes extra features and settings to configure the container with FIPS capabilities. You can configure the next environment variables:
