...
```console
docker run -it --name dex bitmoa/dex
```
...
The recommended way to get the Bitnami Dex Docker Image is to pull the prebuilt image from the [Docker Hub Registry](https://hub.docker.com/r/bitmoa/dex).

```console
docker pull bitmoa/dex:latest
```

To use a specific version, you can pull a versioned tag. You can view the [list of available versions](https://hub.docker.com/r/bitmoa/dex/tags/) in the Docker Hub Registry.

```console
docker pull bitmoa/dex:[TAG]
```
...
```console
docker run --name dex bitmoa/dex:latest
```
...
```console
docker run --rm --name dex bitmoa/dex:latest --help
```
...
The Bitnami Dex Docker image from the [Bitnami Secure Images](https://www.arrow.com/globalecs/uk/products/bitmoa-secure-images/) catalog includes extra features and settings to configure the container with FIPS capabilities. You can configure the next environment variables:
