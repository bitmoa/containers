...
```console
docker run --name harbor-exporter bitmoa/harbor-exporter:latest
```
...
Harbor Exporter is a component of the Harbor application. In order to get the Harbor application running on Kubernetes we encourage you to check the [bitmoa/harbor Helm chart](https://github.com/bitmoa/charts/tree/master/bitmoa/harbor) and configure it using the options exposed in the values.yaml file.
...
|--------------------------------|--------------------------------------------------------------------------------------------|---------------------------------------|
| `HARBOR_EXPORTER_BASE_DIR`     | harbor-exporter installation directory.                                                    | `${BITMOA_ROOT_DIR}/harbor-exporter` |
...
The Bitnami Harbor Exporter Docker image from the [Bitnami Secure Images](https://www.arrow.com/globalecs/uk/products/bitmoa-secure-images/) catalog includes extra features and settings to configure the container with FIPS capabilities. You can configure the next environment variables:
