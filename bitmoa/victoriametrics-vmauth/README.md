...
```console
docker run --name victoriametrics-vmauth bitmoa/victoriametrics-vmauth:latest
```
...
The Bitnami VictoriaMetrics Auth Docker image is only available to [Bitnami Secure Images](https://bitnami.com) customers.
...
```console
docker run --rm --name victoriametrics-vmauth bitmoa/victoriametrics-vmauth:latest --  --help
```
...
The Bitnami VictoriaMetrics Auth Docker image from the [Bitnami Secure Images](https://go-vmware.broadcom.com/contact-us) catalog includes extra features and settings to configure the container with FIPS capabilities. You can configure the next environment variables:
