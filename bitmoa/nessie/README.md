...
```console
docker run -it --name nessie bitmoa/nessie
```
...
The recommended way to get the Bitnami Nessie Docker Image is to pull the prebuilt image from the [Docker Hub Registry](https://hub.docker.com/r/bitmoa/nessie).

```console
docker pull bitmoa/nessie:latest
```

To use a specific version, you can pull a versioned tag. You can view the [list of available versions](https://hub.docker.com/r/bitmoa/nessie/tags/) in the Docker Hub Registry.

```console
docker pull bitmoa/nessie:[TAG]
```
...
or if you're using Docker Compose, update the value of the image property to `bitmoa/nessie:latest`.
...
```console
docker run --name nessie bitmoa/nessie:latest
```
...
The Bitnami Nessie Docker image from the [Bitnami Secure Images](https://www.arrow.com/globalecs/uk/products/bitmoa-secure-images/) catalog includes extra features and settings to configure the container with FIPS capabilities. You can configure the next environment variables:
