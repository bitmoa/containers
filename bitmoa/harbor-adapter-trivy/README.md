...
This container is part of the [Harbor solution](https://github.com/bitmoa/charts/tree/main/bitmoa/harbor) that is primarily intended to be deployed in Kubernetes.

```console
docker run --name harbor-adapter-trivy bitmoa/harbor-adapter-trivy:latest
```
...
The recommended way to get the Bitnami Harbor-Adapter-Trivy Docker Image is to pull the prebuilt image from the [Docker Hub Registry](https://hub.docker.com/r/bitmoa/harbor-adapter-trivy).

```console
docker pull bitmoa/harbor-adapter-trivy:latest
```

To use a specific version, you can pull a versioned tag. You can view the [list of available versions](https://hub.docker.com/r/bitmoa/harbor-adapter-trivy/tags/) in the Docker Hub Registry.

```console
docker pull bitmoa/harbor-adapter-trivy:[TAG]
```
...
```console
docker run \
    -v /path/to/harbor-adapter-trivy-persistence:/bitmoa \
    bitmoa/harbor-adapter-trivy:latest
```
...
```console
docker run --name harbor-adapter-trivy-node1 --network harbor-adapter-trivy-network bitmoa/harbor-adapter-trivy:latest
```
...
Harbor Adapter Trivy is a component of the Harbor application. In order to get the Harbor application running on Kubernetes we encourage you to check the [bitmoa/harbor Helm chart](https://github.com/bitmoa/charts/tree/master/bitmoa/harbor) and configure it using the options exposed in the values.yaml file.
...
|-----------------------------|----------------------------------------------|----------------------------------------------|
| `SCANNER_TRIVY_VOLUME_DIR`  | harbor-adapter-trivy installation directory. | `${BITMOA_VOLUME_DIR}/harbor-adapter-trivy` |
...
|------------------------------|----------------------------------------------|--------------------------------------------|
| `SCANNER_TRIVY_BASE_DIR`     | harbor-adapter-trivy installation directory. | `${BITMOA_ROOT_DIR}/harbor-adapter-trivy` |
...
The Bitnami Harbor Adapter Trivy Docker image from the [Bitnami Secure Images](https://www.arrow.com/globalecs/uk/products/bitmoa-secure-images/) catalog includes extra features and settings to configure the container with FIPS capabilities. You can configure the next environment variables:
