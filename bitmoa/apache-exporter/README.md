...
```console
docker run --name apache-exporter bitmoa/apache-exporter:latest
```
...
The recommended way to get the Bitnami Apache Exporter Docker Image is to pull the prebuilt image from the [Docker Hub Registry](https://hub.docker.com/r/bitmoa/apache-exporter).

```console
docker pull bitmoa/apache-exporter:latest
```

To use a specific version, you can pull a versioned tag. You can view the [list of available versions](https://hub.docker.com/r/bitmoa/apache-exporter/tags/) in the Docker Hub Registry.

```console
docker pull bitmoa/apache-exporter:[TAG]
```
...
```console
docker run --name apache-exporter-node1 --network apache-exporter-network bitmoa/apache-exporter:latest
```
...
The Bitnami Apache Exporter Docker image from the [Bitnami Secure Images](https://www.arrow.com/globalecs/uk/products/bitmoa-secure-images/) catalog includes extra features and settings to configure the container with FIPS capabilities. You can configure the next environment variables:
