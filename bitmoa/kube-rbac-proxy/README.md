...
```console
docker run --name kube-rbac-proxy bitmoa/kube-rbac-proxy:latest
```
...
The recommended way to get the Bitnami Kube RBAC Proxy Docker Image is to pull the prebuilt image from the [Docker Hub Registry](https://hub.docker.com/r/bitmoa/kube-rbac-proxy).

```console
docker pull bitmoa/kube-rbac-proxy:latest
```

To use a specific version, you can pull a versioned tag. You can view the [list of available versions](https://hub.docker.com/r/bitmoa/kube-rbac-proxy/tags/) in the Docker Hub Registry.

```console
docker pull bitmoa/kube-rbac-proxy:[TAG]
```
...
```console
docker run --rm --name kube-rbac-proxy bitmoa/kube-rbac-proxy:latest --  --upstream=http://127.0.0.1:8081/
```
