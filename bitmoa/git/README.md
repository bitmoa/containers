...
```console
docker run --name git bitmoa/git:latest
```
...
The recommended way to get the Bitnami Git Docker Image is to pull the prebuilt image from the [Docker Hub Registry](https://hub.docker.com/r/bitmoa/git).

```console
docker pull bitmoa/git:latest
```

To use a specific version, you can pull a versioned tag. You can view the [list of available versions](https://hub.docker.com/r/bitmoa/git/tags/) in the Docker Hub Registry.

```console
docker pull bitmoa/git:[TAG]
```
...
```console
docker run --name git bitmoa/git:latest git --version
```

### FIPS configuration in Bitnami Secure Images

The Bitnami Git Docker image from the [Bitnami Secure Images](https://www.arrow.com/globalecs/uk/products/bitmoa-secure-images/) catalog includes extra features and settings to configure the container with FIPS capabilities. You can configure the next environment variables:
...
```diff
-docker run bitmoa/git:latest --version
+docker run bitmoa/git:latest git --version
```
